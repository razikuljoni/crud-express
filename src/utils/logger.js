...
const SENSITIVE = [
    "password","pass","passwd","token","secret","key",
    "authorization","auth","cookie","creditcard","cardnumber",
    "cvv","ssn","socialsecurity","api_key","apikey",
    "private_key","privatekey","seed","mnemonic",
];
...
const consoleFmt = printf((info) => {
    ...
    for (const [k, v] of Object.entries(meta)) {
        if (!["level","timestamp","message","service","env","hostname","pid"].includes(k)) clean[k] = v;
    }
    ...
    let status = "";
    if (statusCode) {
        const c = statusCode >= 500 ? A.RED : statusCode >= 400 ? A.YEL : statusCode >= 300 ? A.CYN : A.GRN;
...
const jsonFmt = printf((info) => {
    const out = {
        ts: info.timestamp, level: info.level,
        service: SERVICE, env: ENV, hostname: HOSTNAME, pid: PID,
        msg: info.message,
    };
    ...
    const skip = new Set([
        "timestamp","level","message","stack","statusCode","method",
        "url","requestId","duration","ip","service","env","hostname","pid",
    ]);
    const extra = {};
...
if (!cleanupScheduled) {
    ...
    setTimeout(() => { enforceRetention(); setInterval(enforceRetention, 86400000); }, 5000);
}

export { redact, enforceRetention, RETENTION_DAYS, MAX_SIZE };
